...
<nav className="bg-white shadow-md fixed top-0 z-50 w-full">
  <div className="container mx-auto flex items-center justify-between py-3 sm:py-4 md:py-6 px-4 sm:px-8 md:px-16 lg:px-22">
    ...
    <div className="hidden md:flex md:space-x-8 lg:space-x-16">
      <Link to="/user/home" className="uppercase text-black font-bold tracking-wide text-xs lg:text-sm hover:text-[#1C359A] transition-colors">Home</Link>
      <Link to="/user/menu" className="uppercase text-black font-bold tracking-wide text-xs lg:text-sm hover:text-[#1C359A] transition-colors">Menu</Link>
      <Link to="/user/company" className="uppercase text-black font-bold tracking-wide text-xs lg:text-sm hover:text-[#1C359A] transition-colors">Company</Link>
      <Link to="/user/special" className="uppercase text-black font-bold tracking-wide text-xs lg:text-sm hover:text-[#1C359A] transition-colors">Special</Link>
      <Link to="/user/contact" className="uppercase text-black font-bold tracking-wide text-xs lg:text-sm hover:text-[#1C359A] transition-colors">Contact</Link>
    </div>
        ...
        <div className="flex flex-col p-4 space-y-4">
          ...
          <div className="flex flex-col gap-6 w-full py-2 items-center justify-center">
            <div className="flex flex-row justify-center items-center gap-4">
              <img src="../img/user.png" className="w-5 h-5 sm:w-6 sm:h-6" alt="User icon" />
              <Link to="/user/account" className="uppercase text-[#1C359A] font-bold text-sm">Account</Link>
            </div>
            <div className="flex flex-row justify-center items-center gap-4">
              <img src="../img/logout.png" className="w-5 h-5 sm:w-6 sm:h-6" alt="Logout icon" />
    ...
    <div className="gap-3 md:gap-5 items-center hidden md:flex">
      ...
      <div className="relative">
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 md:w-4 md:h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="flex" ref={dropdownRef}>
        ...
        {/* Dropdown Menu */}
        {isDropdownVisible && (
          <div id="profileDropdown" className="absolute right-4 sm:right-8 md:right-16 lg:right-22 mt-10 w-56 md:w-64 bg-white shadow-lg rounded-lg border border-gray-200 z-50">
            {/* Profile Picture & Username Section */}
            <div className="p-3 border-b border-gray-300">
              <div className="flex items-center">
                <div id="profilePicture" className="w-8 h-8 md:w-10 md:h-10 bg-gray-300 rounded-full overflow-hidden">
                  {hasValidProfilePic() ? (
                    <img 
                      src={getProfilePicUrl()}
                      alt="Profile" 
                ...
                <div className="ml-3">
                  <Link to="/user/account" className="text-xs md:text-sm font-medium text-gray-800">
                    {user ? `${user.f_name} ${user.l_name}` : "Guest"}
                  </Link>
                </div>
              </div>
            </div>
            <Link to="/user/account" className="block px-4 py-2 text-xs md:text-sm text-gray-800 hover:bg-gray-200">Account</Link>
            <Link to={{ pathname: '/user/cart', state: { cartItems } }} className="block px-4 py-2 text-xs md:text-sm text-gray-800 hover:bg-gray-200">My Cart</Link>
            <Link to="/" onClick={logoutUser} className="block px-4 py-2 text-xs md:text-sm text-gray-800 hover:bg-gray-200">Log Out</Link>
          </div>
        )}
      </div>
    </div>
    ...
    {/* Notification Panel */}
    {isNotificationOpen && (
      <div ref={notificationRef} className="absolute top-full right-4 sm:right-8 md:right-16 lg:right-36 w-72 sm:w-80 bg-white shadow-lg rounded-lg border border-gray-200 z-50 mt-1 md:mt-0">
        <div className="p-3 md:p-4 border-b border-gray-400 flex justify-between items-center">
          <span className="font-semibold text-xs md:text-sm">Order Notifications</span>
          <Link to="/user/status" className="text-[#1C359A] text-xs md:text-sm">See more...</Link>
        </div>
        <div className="max-h-48 md:max-h-60 overflow-y-auto">
          {notifications.length > 0 ? (
            ...
            .map((notification) => (
              <div key={notification.orders_id} className="p-3 md:p-4 border-b">
                <span className="font-semibold text-xs md:text-sm">Order Status</span>
                <p className="text-xs md:text-sm text-gray-600">Your order #{notification.orders_id} is {notification.order_status}</p>
              </div>
            ))
          ) : (
            <p className="p-3 md:p-4 text-gray-500 text-center text-xs md:text-sm">No new notifications</p>
          )}
        </div>
      </div>
    )}
